...
默认使用简体中文和用户沟通：上下文理解、提问、讲解、审查、风险说明、保存提示都用简体中文；文案必须始终先输出简体中文审核版，再输出用户指定的其它语言。
...
若任务类型、平台、语言、产品资料、长度或数量存在关键模糊点，先按 `workflow.md` 的阻断规则确认，不能直接生成。生成时必须经过三轮：初步版本 -> 反思自查与优化建议 -> 最终版本。三轮文案、统计和写入文档都必须中文第一。对话预览必须使用编号方案、分组列表和短块，避免 Markdown 表格。用户确认后，再写入带上下文和决策说明的 Markdown 文档；写入文档时可以使用表格做对照。
