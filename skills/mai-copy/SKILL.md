...
8. 若用于图片、A+、包装或详情页固定版式，必须检查最长单词；德语等语言避免长复合词撑破版式。

## 输出前置规则

若任务类型、平台、语言、产品资料、长度或数量存在关键模糊点，先按 `workflow.md` 的阻断规则确认，不能直接生成。生成时必须经过五步双反思：初步版本 -> 第一次反思 -> 修正版 -> 第二次反思 -> 最终版本。五步文案、统计和写入文档都必须中文第一。对话预览必须使用编号方案、分组列表和短块，避免 Markdown 表格。用户确认后，再写入带上下文和决策说明的 Markdown 文档；写入文档时可以使用表格做对照。
