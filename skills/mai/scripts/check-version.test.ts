...
describe("check installed version", () => {
  ...
  test("reports version and workflow markers", async () => {
    ...
    await writeFile(join(root, "VERSION"), "1.1.5\n");
    await writeFile(
      join(root, "references", "workflow.md"),
      "双反思产出硬约束\n命令行可读性硬约束\n中文优先硬约束\n长单词排版硬约束\n",
    );
    await writeFile(join(root, "LATEST_VERSION"), "1.1.5\n");

    const check = await checkInstalledVersion(root, { latestVersion: "1.1.5" });

    expect(check.version).toBe("1.1.5");
    expect(check.latestVersion).toBe("1.1.5");
    ...
    expect(renderVersionCheck(check)).toContain("Mai installed version: 1.1.5");
    expect(renderVersionCheck(check)).toContain("Latest GitHub version: 1.1.5");
  ...
  test("shell version check runs without Bun or Python", async () => {
    ...
    expect(code).toBe(0);
    expect(`${stdout}\n${stderr}`).toContain("Mai installed version: 1.1.5");
    expect(stdout).toContain("Latest GitHub version:");
    expect(stdout).toContain("Latest GitHub version: 1.1.5");
