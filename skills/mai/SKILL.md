...
2. 文案语言必须中文优先：无论用户是否要求中文，`第一轮`、`第二轮`、`第三轮`、统计和写入文档都必须先输出简体中文审核版，再输出用户指定的其它语言。
3. 用户可完全使用自然语言；不要要求普通用户记参数名。
4. 缺少关键参数时，一次只问一个数字选择题，用户回复数字即可。
5. 若需求边界模糊或上下文冲突，必须先确认，不能直接生成。
6. 每次文案生成必须经过三轮：初步版本 -> 反思自查与优化建议 -> 最终版本。
7. 先理解和确认场景、语言、数量/版式约束。
8. 先在对话中用分组列表列出文案预览和统计，不直接写文档，不使用 Markdown 表格。
9. 用户确认后，才把带上下文、决策过程、最终结论和统计的 Markdown 文档写入文件。
